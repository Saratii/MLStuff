...
pixelSize = 28
traindata = pd.read_csv('CSV_Files/mnist_train.csv')
# testdata = pd.read_csv('CSV_Files/mnist_test.csv')
# customData = pd.read_csv('CSV_Files/customDrawing.csv')
animalData = pd.read_csv('CSV_Files/AnimalData.csv')
def train(CSVData, inputCount, layer1NeuronCount, numClasses, numIterations):
    retrain = True
    ...
    m, x_train, y_train, data_train, x_dev, y_dev = processData(CSVData)
    ...
    def initParams():
        ...
        w2 = np.random.rand(numClasses, layer1NeuronCount) - 0.5
        b2 = np.random.rand(numClasses, 1) - 0.5
        return w1, b1, w2, b2
    def relu(z):
    ...
    def testPredictions(index, w1, b1, w2, b2):
        ...
        currentImage = currentImage.reshape((pixelSize, pixelSize)) * 255
        plt.gray()
    ...
    for i in range(len(x_train)-1):
        testPredictions(i, w1, b1, w2, b2)
    
classes = 10
neurons = 10
iterations = 100
train(traindata, pixelSize**2, classes, neurons, iterations)
