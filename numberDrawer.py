...
squareSize = 20
(width, height) = (28*squareSize, 28*squareSize)
screen = pygame.display.set_mode((width, height))
pygame.display.set_caption('Manual Data Generation')
...
for i in range(28):
    for j in range(28):
        squares.append(square(i*squareSize, j*squareSize))

while running:
    x, y = pygame.mouse.get_pos()
    
    for squar in squares:
        pygame.draw.rect(screen, (squar.r, squar.b, squar.g), pygame.Rect(squar.x, squar.y, squareSize, squareSize))
    
    if mouse == "down":
        for squar in squares:
            if squar.x<=x<squar.x+squareSize*1.5 and squar.y<=y<squar.y+squareSize*1.5:
                squar.r = 255
                squar.g = 255
                squar.b = 255
            elif squar.x<=x<squar.x+squareSize*3 and squar.y<=y<squar.y+squareSize*3 and squar.r != 255:
